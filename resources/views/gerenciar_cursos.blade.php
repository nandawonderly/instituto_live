...
<main class="content-area"> 
    ...
    <div class="cursos-grid">
        ...
        <div class="card-curso" style="border: 1px solid #eaeaea; padding: 20px; border-radius: 8px; margin-bottom: 15px; display: flex; justify-content: space-between; align-items: center;">
            <div>
                <h3 style="color: #1c4a8a; margin-bottom: 5px;">{{ $curso->nome }}</h3>
                <p style="color: #666; font-size: 14px;">{{ $curso->descricao }}</p>
            </div style="display: flex; gap: 10px;">
            <form action="/admin/cursos/{{ $curso->id }}" method="POST" onsubmit="return confirm('Apagar este curso apagará TUDO dentro dele. Tem certeza?');" 
            ...
            <a href="/admin/cursos/{{ $curso->id }}" class="btn-editar" style="text-decoration: none; background-color: #1c4a8a; color: white; padding: 8px 15px;">Ver Disciplinas</a>
        </div>
        @endforeach
    </div>
</main>
